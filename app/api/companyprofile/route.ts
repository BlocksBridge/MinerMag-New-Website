...
if (companySymbol.has('company')) {
        let companyProfile  = await supabase.from("marketdata").select("").eq("slug", `companyprofile/${companySymbol.get("company")?.toUpperCase()}`)
        if (companyProfile.error || !companyProfile.data.length) {
            return NextResponse.json({error: "Something went wrong"})
        } else {
            return NextResponse.json({data:JSON.parse(companyProfile.data[0].data_points)})
        }
} else {
