...
if (companySymbol.has('company')) {
        let getEnterpriseData  = await supabase.from("marketdata").select("").eq("slug", `enterprisevalue/${companySymbol.get("company")?.toUpperCase()}`)
        if (getEnterpriseData.error || !getEnterpriseData.data.length) {
            return NextResponse.json({error: "Something went wrong"})
        } else {
            return NextResponse.json({data:JSON.parse(getEnterpriseData.data[0].data_points)})
        }
} else {
